...
%  Matrices del sistema de Biot: A11, A12, A21 y A22
    % Para el presente problema, solamente nos interesan A11 y A12
[A11,A12,~,~,~]=build_matrices_Biot(delta_t);

% Initialize the vector for the solution of the pressure 
p=zeros(N*N,1);
...
% We solve the system E*u = g -G*p: Time loop

while t < Tf
    
    % Exact solution for the pressure at time t+delta_t
for j=1:N
    for i=1:N
        ind1p=i+(j-1)*N;
        ...
        p(ind1p)=sol_exactax(xx,yy,t+delta_t,3);
    end
end

    % Source term for Elasticity at t+delta_t
...
[gDu,~]=dir_bc_Pg(delta_t,t+delta_t);   
f_hat= -f_indep + gDu; 

    % R.H.S. term
indep_term= f_hat - A12*p;
    
% Solution of the system for the displacement vector
u=A11\indep_term; 
