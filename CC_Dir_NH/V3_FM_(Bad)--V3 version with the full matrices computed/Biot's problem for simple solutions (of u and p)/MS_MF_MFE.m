...
% Matrices completas del sistema de Biot
Ass=build_matrix_sig_sig;
AsuT=build_matrix_sig_u;
Asu=AsuT';
AsgT=build_matrix_sig_gamma;
Asg=AsgT';
AspT=build_matrix_sig_p;
Asp=AspT';
Azz=build_matrix_z_z;
AzpT=build_matrix_z_p;
Azp=delta_t*AzpT';
App=build_matrix_p_p;

%  Matrices del sistema de Biot: A11, A12, A21 y A22
% A11=Asu*(Ass\AsuT)-((Asu*(Ass\AsgT))*((Asg*(Ass\AsgT))\(Asg*(Ass\AsuT))));
A11=Asu*inv(Ass)*AsuT - Asu*inv(Ass)*AsgT*inv(Asg*inv(Ass)*AsgT)*Asg*inv(Ass)*AsuT;
% A12=Asu*(Ass\AspT)-((Asu*(Ass\AsgT))*((Asg*(Ass\AsgT))\(Asg*(Ass\AspT))));
A12=Asu*inv(Ass)*AspT - Asu*inv(Ass)*AsgT*inv(Asg*inv(Ass)*AsgT)*Asg*inv(Ass)*AspT;
% A21=-A12';
A21=-Asp*inv(Ass)*AsuT + Asp*inv(Ass)*AsgT*inv(Asg*inv(Ass)*AsgT)*Asg*inv(Ass)*AsuT;
% A22=App-Asp*(Ass\AspT)+Azp*(Azz\AzpT)+((Asp*(Ass\AsgT))*((Asg*(Ass\AsgT))\(Asg*(Ass\AspT))));
A22=App - Asp*inv(Ass)*AspT + Azp*inv(Azz)*AzpT + Asp*inv(Ass)*AsgT*inv(Asg*inv(Ass)*AsgT)*Asg*inv(Ass)*AspT;
...
[Pgu,~]=compute_bc(t);
% gamma=(Asg*(Ass\AsgT))\(Asg*(Ass\Pgu)-Asg*(Ass\AsuT)*u-Asg*(Ass\AspT)*p);  
gamma=inv(Asg*inv(Ass)*AsgT)*(Asg*inv(Ass)*Pgu - Asg*inv(Ass)*AsuT*u - Asg*inv(Ass)*AspT*p);
% sigma=Ass\(Pgu-AsuT*u-AsgT*gamma-AspT*p);
sigma=inv(Ass)*(Pgu - AsuT*u - AsgT*gamma - AspT*p);
...
[Pgu,Pgp]=compute_bc(t+delta_t);   
% gDu=(Asu-(Asu*(Ass\AsgT))*((Asg*(Ass\AsgT))\Asg))*(Ass\Pgu);
gDu=(Asu - Asu*inv(Ass)*AsgT*inv(Asg*inv(Ass)*AsgT)*Asg)*inv(Ass)*Pgu;
% gDp=Azp*(Azz\Pgp)+(-Asp+(Asp*(Ass\AsgT)*((Asg*(Ass\AsgT))\Asg)))*(Ass\Pgu);
gDp=Azp*inv(Azz)*Pgp + (-Asp + Asp*inv(Ass)*AsgT*inv(Asg*inv(Ass)*AsgT)*Asg)*inv(Ass)*Pgu;
f_hat= -f_indep + gDu; 
