function indep1=build_indep_f0(tt)
...
%         lambda=E*c_lambda;
%         mu=E*c_mu;      
        indep1(ind1)=int_f_simpson(i,j,1,tt);
        indep1(ind2)=int_f_simpson(i,j,2,tt);
    end
end
return
end

function intt=int_f_simpson(i,j,k,tt)

intt=1/36*(g(i,j,0,0,k,tt)+4*g(i,j,0.5,0,k,tt)+g(i,j,1,0,k,tt)+...
            4*(g(i,j,0,0.5,k,tt)+4*g(i,j,0.5,0.5,k,tt)+g(i,j,1,0.5,k,tt))+...
            g(i,j,0,1,k,tt)+4*g(i,j,0.5,1,k,tt)+g(i,j,1,1,k,tt));
...
gg=jacob*f0(x(i,j)+(x(i+1,j)-x(i,j))*xx+(x(i,j+1)-x(i,j))*yy+(x(i+1,j+1)-x(i,j+1)-x(i+1,j)+x(i,j))*xx*yy,...
